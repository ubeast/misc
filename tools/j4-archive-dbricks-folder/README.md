...
One file: [`j4_archive_dbricks_folder.py`](j4_archive_dbricks_folder.py). Unlike
...
1. Get `j4_archive_dbricks_folder.py` into the Databricks workspace folder you
...
```bash
uv run pytest tools/j4-archive-dbricks-folder        # or: pytest tools/j4-archive-dbricks-folder
```
...
`python tools/j4-archive-dbricks-folder/tests/regenerate_samples.py`.
...
3. Copy `j4_archive_dbricks_folder.py` into that folder (keep the
...
This tool had its own repository (`ubeast/j4_archive_dbricks_folder`) through
2026-09. Its pre-move changelog lives in that repo's git history.
