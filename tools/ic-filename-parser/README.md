...
It is still one file: [`ic_filename_parser.py`](ic_filename_parser.py), standard
library only (no pandas). Copy it wherever you need it — no install.
...
`tests/` runs against the script via `tests/conftest.py`, which puts this
directory on the path. From the repo root:

```bash
uv run pytest tools/ic-filename-parser        # or: pytest tools/ic-filename-parser
```
